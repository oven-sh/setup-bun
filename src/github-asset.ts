...
function parseAssetUrl(downloadUrl: string): UrlAssetMetadata {
  ...
  const parts = urlObj.pathname.slice(1).split("/").map(decodeURIComponent);
  if (parts.length !== 6) {
...
export async function fetchAssetMetadata(
  ...
  const releasePath = base.latest
    ? "latest"
    : `tags/${encodeURIComponent(base.tag)}`;
