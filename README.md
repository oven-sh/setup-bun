...
| `token`            | Personal access token (PAT) used to fetch tags from the `oven-sh/bun` repository. Recommended for resolving wildcard/range versions to avoid GitHub API rate limiting. | `${{ github.token }}` | `${{ secrets.GITHUB_TOKEN }}`                    |
